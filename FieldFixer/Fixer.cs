...
namespace FieldFixer
{
	...
	public class Fixer
	{
		...
		private void map_protobuf_type(TypeReference obf, TypeReference de, bool map_constants = true) {
			...
			for (uint i = 0; i < field_count; i++) {
				...
				if (!ob_field_v.Equals(de_field_v)) {
					WriteLine("WARN: fields {0} and {1} have incompatible variety: {2} and {3}! Skipping mapping", ob_field.name, de_field.name, ob_field_v, de_field_v);
				} else {
		...
		private SortedDictionary<uint, ProtobufField> get_protobuf_fields(TypeReference t) {	
			...
			var fields = type.Fields.OrderBy(x => GetFieldToken(x)).ToList();
			...
			for (int i = 0; i < fields.Count-1; i++) {
				...
				if (is_protobuf_field(f1)) {
					// Protobuf ID
					var f2 = fields[i+1];
					if (f2.IsPrivate && !f2.HasConstant) {
						// Regular field
						var ft = f2.FieldType;
		...
		private uint GetFieldToken(FieldDefinition f)
		{
